...
import { displayDate, template, escape as e } from "../lib/html.js";
...
export function Availability(t) {
  return `<section class="section availability-section" id="availability"><div>${sectionHeading(t.availability.eyebrow, t.availability.title)}${badge(t)}<p>${t.availability.intro}</p><p class="small-copy">${t.status.noDate && !apartment.availability.availableFrom ? t.status.noDate : ""}</p><div class="calendar-legend">${["available", "reserved", "occupied", "unknown"].map((s) => `<span><i class="${s}"></i>${t.status[s]}</span>`).join("")}</div><p class="small-copy">${t.availability.hint}</p></div><div class="calendar-panel"><div id="calendar" role="group" aria-label="${t.availability.calendar}">${calendarMarkup(t)}</div><p class="small-copy">${template(t.availability.select, { minimum: apartment.rental.minimumMonths })}</p><div class="date-inputs"><label>${t.availability.start}<input type="date" id="availability-start" min="${isoDate()}"></label><label>${t.availability.end}<input type="date" id="availability-end" min="${isoDate()}"></label></div><p id="calendar-status" role="status">${t.availability.empty}</p><button id="ask-dates" class="button">${t.availability.ask} <span aria-hidden="true">↗</span></button><button id="clear-dates" class="text-button">${t.availability.reset}</button></div></section>`;
}
